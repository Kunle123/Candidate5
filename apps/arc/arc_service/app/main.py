...
def parse_cv_with_ai_chunk(text):
    ...
    prompt_instructions = (
        ...
        "- For each job role (work experience), extract every sentence, bullet point, and relevant detail, even if they appear in different sections or are phrased differently. Do not omit any information. If a job is mentioned in multiple places, combine all details into a single entry for that job.\n"
        "- For each job, include: company, title, start_date, end_date, and a detailed description (with all sentences and bullet points).\n"
